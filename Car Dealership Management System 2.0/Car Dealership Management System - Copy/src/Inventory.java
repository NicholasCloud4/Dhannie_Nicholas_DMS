...
public class Inventory {
    ...
    public void addCar(Car car) {
        cars.add(car);
        lastAssignedId = car.getId();
    }

    private int lastAssignedId = 0;
    public int getNextAvailableId() {
        return lastAssignedId + 1; // Return the next available ID
    }
     ...
     * Name: removeCarById
     *
     * @param id This will remove the car by if the user selects to remove by ID
     *           EX: id, 4
     * @return
     */
    ...
    public boolean containsCarId(int carId) {
        for (Car car : cars) {
            if (car.getId() == carId) {
                return true; // Car ID found in the inventory
            }
        }
        return false; // Car ID not found in the inventory
    }
